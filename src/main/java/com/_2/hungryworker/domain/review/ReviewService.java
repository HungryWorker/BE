...
public class ReviewService {
    ...
    public List<ReviewResponse> getReviews(Long restaurantId, Long viewerId) {
        return reviewRepository.findByRestaurantIdOrderByCreatedAtDesc(restaurantId).stream()
                .map(review -> ReviewResponse.of(review, isMine(review, viewerId)))
                .toList();
    }
    ...
    public ReviewSummaryResponse getSummary(Long restaurantId) {
        ...
        double totalScore = reviewRepository.findAverageRatingByRestaurantId(restaurantId).orElse(0.0);
        double avgTaste = reviewRepository.findAverageTasteScoreByRestaurantId(restaurantId).orElse(0.0);
        double avgPortion = reviewRepository.findAveragePortionScoreByRestaurantId(restaurantId).orElse(0.0);
        double avgValue = reviewRepository.findAverageValueScoreByRestaurantId(restaurantId).orElse(0.0);
        double avgHygiene = reviewRepository.findAverageHygieneScoreByRestaurantId(restaurantId).orElse(0.0);
    ...
    @Transactional
    public ReviewResponse createReview(Long userId, Long restaurantId, ReviewCreateRequest request,
        ...
        validateNotEmpty(content, validImages, taste, portion, value, hygiene);
        ...
        return ReviewResponse.of(saved, true);
    }
    ...
    public ReviewResponse updateReview(Long userId, Long reviewId, ReviewUpdateRequest request) {
        ...
        // 수정 시에는 기존 첨부 이미지를 그대로 유지하므로, 이미지가 남아있다면
        // 댓글/별점이 모두 비어도 리뷰 자체는 유효하다.
        if (!hasExistingImages) {
            validateNotEmpty(content, List.of(), taste, portion, value, hygiene);
        }

        review.update(content, taste, portion, value, hygiene);
    ...
    public void deleteReview(Long userId, Long reviewId) {
        ...
        review.getImages().forEach(image -> fileStorageService.delete(image.getImageUrl()));
        reviewRepository.delete(review);
    }

    private void attachImages(Review review, List<MultipartFile> images) {
        for (MultipartFile image : images) {
            String storedUrl = fileStorageService.store(image, REVIEW_IMAGE_SUB_DIR);
    ...
    private void validateNotEmpty(String content, List<MultipartFile> images,
                                  double taste, double portion, double value, double hygiene) {
        boolean hasContent = content != null;
        boolean hasImages = !images.isEmpty();
        boolean hasScore = taste > 0 || portion > 0 || value > 0 || hygiene > 0;

        if (!hasContent && !hasImages && !hasScore) {
            throw new IllegalArgumentException("댓글, 사진, 별점 중 하나는 반드시 입력해야 합니다.");
        }
    }

    /**
     * 별점은 0~5 사이, 0.5 단위로만 허용한다. null이면 "입력하지 않음"으로 보고 0점으로 취급한다.
     */
    private double validateScore(Double score, String label) {
        if (score == null) {
            return 0.0;
        }
        if (score < 0 || score > 5) {
            throw new IllegalArgumentException(label + " 별점은 0~5 사이여야 합니다.");
        }

        double doubled = score * 2;
        if (Math.abs(doubled - Math.round(doubled)) > 1e-6) {
            throw new IllegalArgumentException(label + " 별점은 0.5 단위로만 입력할 수 있습니다.");
        }
        return Math.round(doubled) / 2.0;
    }

    private boolean isMine(Review review, Long viewerId) {
        return viewerId != null && review.getUser().getId().equals(viewerId);
    }

    private void validateOwner(Review review, Long userId) {
        if (!review.getUser().getId().equals(userId)) {
            throw new IllegalArgumentException("본인이 작성한 리뷰만 수정/삭제할 수 있습니다.");
        }
    }
